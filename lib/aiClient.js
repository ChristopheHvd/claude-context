...
/**
 * @param {object} context — output of buildContext / truncateForPrompt
 * @param {string} apiKey
 * @param {string} providerId — one of: anthropic, openai, gemini, perplexity
 * @returns {Promise<string>}
 */
export async function generateProfile(context, apiKey, providerId = 'anthropic') {
  const provider = getProvider(providerId);
  const prompt = buildPrompt(context);
  const { url, headers, body } = provider.buildRequest(prompt, apiKey, provider.model);
  ...
  if (!res.ok) {
    ...
    throw new Error(provider.parseError(err) || `API error ${res.status}`);
  }
